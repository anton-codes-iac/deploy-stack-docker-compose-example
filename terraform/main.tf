...
resource "aws_cloudwatch_log_group" "app_logs" {
  name              = "/ecs/deploy-stack-docker-compose-example"
  retention_in_days = 14
}

# --- ECR Repository ---
resource "aws_ecr_repository" "app" {
  name                 = "deploy-stack-docker-compose-example-repo"
...
resource "aws_iam_role" "execution_role" {
  name               = "deploy-stack-docker-compose-example-execution-role"
...
resource "aws_iam_role" "task_role" {
  name               = "deploy-stack-docker-compose-example-task-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_trust.json
}

# --- ECS Cluster ---
resource "aws_ecs_cluster" "main" {
  name = "deploy-stack-docker-compose-example-cluster"
}

# --- ECS Task Definition ---
resource "aws_ecs_task_definition" "app" {
  family                   = "deploy-stack-docker-compose-example-task"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name      = "deploy-stack-docker-compose-example-container"
...
resource "aws_lb" "main" {
  name               = "deploy-stack-docker-compose-alb"
...
resource "aws_lb_target_group" "app" {
  name        = "deploy-stack-docker-compose-tg"
  port        = 8080
  protocol    = "HTTP"
...
resource "aws_ecs_service" "app" {
  name            = "deploy-stack-docker-compose-example-service"
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.app.arn
    container_name   = "deploy-stack-docker-compose-example-container"
    container_port   = 8080
  }
...
resource "aws_iam_role_policy" "secrets_policy" {
  name   = "deploy-stack-docker-compose-example-secrets-policy"
...
resource "aws_cloudwatch_metric_alarm" "alb_5xx_errors" {
  alarm_name          = "deploy-stack-docker-compose-example-high-5xx-errors"
